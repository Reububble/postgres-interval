(module
  ...
  ;; input utf16 (with stride 4) null terminated string to parse in memory
  ...
  (func $parse
    ...
    loop $a
      global.get $pos i32.load local.tee $chr ;; chr = mem[pos]
      
      i32.eqz if return end ;; if (chr === 0) return;

      local.get $chr i32.const 45 i32.eq if ;; if (chr === '-')
        i32.const -1 local.set $sig ;; sig = -1
        i32.const 4 global.get $pos i32.add global.set $pos ;; ++pos
        ...
        i32.const 58 i32.eq if (param i32);; if (chr === ':')
          ...
          i32.const 4 global.get $pos i32.add global.set $pos ;; ++pos
          global.get $pos i32.load call $readNextNum local.get $sig i32.mul global.set $min ;; min = sig * readNextNum(mem[pos])

          i32.const 4 global.get $pos i32.add global.set $pos ;; ++pos
          global.get $pos i32.load call $readNextNum local.get $sig i32.mul global.set $sec ;; sec = sig * readNextNum(mem[pos])
  ...
  (func $readNextNum (param $chr i32) (result i32)
    ...
    loop $a
      global.get $pos i32.const 4 i32.add global.set $pos ;; ++pos
      global.get $pos i32.load local.tee $chr ;; chr = mem[pos]
